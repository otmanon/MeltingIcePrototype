...
![Heat diffusing. Each cell tries to take the temperature of it's neighborhood. Eventually, heat will have completely diffused and the system will be in steady state, where each cell
is as close to it's neighboring cells' temperatures as possible](https://i.imgur.com/3AMdJOq.gif)
...
All the code is written in matlab. You need [gptoolbox](https://github.com/alecjacobson/gptoolbox) as well as the triangle extension. To find details on how to install the triangle extension, [check it out here](https://dgptimbits.wordpress.com/2017/07/20/installing-the-triangle-package-for-gptoolbox/). If you're on windows, note I attached a precompiled triangle binary to my submission. Just go to `path_to_triangle.m` and change the path defined in variable `s` to be where `s = ./`
